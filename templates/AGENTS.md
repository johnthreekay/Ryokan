...
- **`hx-vals="js:…"` is an object-literal body, not an expression.** htmx 4 takes the text after `js:` and, unless it starts with `{`, wraps it in braces before evaluating, so `js:someFn(this)` becomes `{someFn(this)}` and dies with "SyntaxError: missing formal parameter" (the library page's sort controls hit this). Write the literal, or spread a call into one: `hx-vals="js:{...librarySortParams('key', this)}"`. `this` is the element either way.
- **Empty form fields ride along on `hx-get`.** A filter form sends every field, so an untouched select or search box pushes `&category=&search=` into the URL. `hx-on::config:request` receives `event.detail.ctx.request.body` (the FormData the query is built from) before the request goes out; deleting empty entries there keeps the pushed URL clean (`dropEmptyLogParams` in `system.js`, used by the Logs filter and jump forms).
