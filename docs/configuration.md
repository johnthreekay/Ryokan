...
- **Upgrade Until Custom Format Score**: a file at the quality cutoff is still replaced by a same-quality release with a higher Custom Format score, until the file's score reaches this. 0 (the default) turns score-only upgrades off for any file scoring 0 or better; TRaSH-style setups raise it.
