...
Episode numbers use the same parser as the rest of Ryokan, so the `E07` in the preview is the number the import would record. A file that holds several episodes (`Title - 05-06.mkv`, `S01E05-E06`) shows the range, `E05-E06`, and is recorded for each of them. Files that Ryokan cannot number (creditless openings and endings, bare specials, files with no digits) are listed but marked **No episode number** and would not be imported.
...
- **Special**: the name marks an OVA or special (`OVA 01`, `SP1`) and the match is a TV series, so the file is not one of its episodes. It is written into the series' `Specials` folder under its own name and gets no episode entry. For a series that is itself an OVA the marker is part of its name, and the file imports as an ordinary episode.
