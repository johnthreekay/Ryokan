...
- **Revision**: a v2, PROPER or REPACK of the same release, from the same group that made the file. Groups put out a v2 to fix a mistake, so it replaces the file even when the cutoff is already met. Through RSS this only happens while the file is under a week old. The daily upgrade search only looks at files below your quality cutoff, so a fix for a file that already meets it comes through RSS or not at all. A v2 from a different group is not a fix of your file and is left alone.
- **Custom Format score**: a same-quality release whose Custom Formats score higher, until the file's score reaches **Upgrade Until Custom Format Score**. The gain has to be at least the **Minimum Custom Format Score Increment**. The default cutoff of 0 turns this off for any file scoring 0 or better. These arrive through RSS; the daily upgrade search only looks for files below the quality cutoff. A file Ryokan did not grab itself has no release title to score and is only replaced for quality.

A v2 also scores a little higher than the plain release when both are on offer, so the fixed one wins the tie. All of this follows **Propers and Repacks** on Quality & Releases: **Prefer and upgrade** (the default), **Prefer, do not upgrade**, or **Do not prefer**. The reason a release was not taken is written to the RSS decision log and the auto-search log.
...
Fansub groups often number a sequel as if the show never ended, so "Jujutsu Kaisen - 56" is season 3, episode 9. Ryokan works out how many episodes came before from AniList's sequel chain and searches under the earlier seasons' names as well. Where that chain is missing a link, as with Dragon Ball Kai and its 2014 continuation, it uses [anime-relations](https://github.com/erengy/anime-relations), the community-maintained list behind Taiga. A copy ships inside Ryokan and is updated with each release.
